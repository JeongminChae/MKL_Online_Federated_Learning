...
function [theta_nodes,er_hat_nodes,Consensus_all_nodes,q_dis_nodes,Eff_all_nodes,erm_mean_nodes1,erm_mean_nodes2,erm_mean_nodes3,erm_mean_nodes4,erm_mean_nodes5]=Damkl_online_rf(y,X,params,Connectivity)
...
%for i = 1:n_nodes 
%    B_X{i} = X(:,1+(i-1)*n_data:n_data+(i-1)*n_data); % Data for nodes
%    B_y{i} = y(1+(i-1)*n_data:n_data+(i-1)*n_data); % Label for nodes
%end 
...
for t=1:n_data
    ...
    for i = 1:n_nodes
        ...
        %if value_c < eta_c && sum(Eff_node([t-M:t-1])) < M
        %    Eff_node(t)=1;
        %end
